...
# Project: Frailty_Komoto 2021 Medicare FFS Table 1
# Author: Nemo Zhou
...
table1_cohort <- "table1_2021_medicare_ffs_cohort"
min_count <- 11L
...
# ---- Identify the 2021 primary Medicare FFS subgroup ----
...
write.csv(
  medicare_ffs_count,
  file.path(output_dir, "2.5_medicare_ffs_count_2021.csv"),
  row.names = FALSE,
  na = ""
)

# ---- Materialize the subgroup required by k_table1() ----
message("Preparing the 2021 primary Medicare FFS Table 1 cohort.")
...
table1_2021_medicare_ffs <- tryCatch(
  {
    message("Generating Table 1 for the 2021 primary Medicare FFS subgroup.")
...
print(table1_2021_medicare_ffs)

write.csv(
  as.data.frame(table1_2021_medicare_ffs),
  file.path(output_dir, "2.5_table1_2021_medicare_ffs.csv"),
  row.names = FALSE,
  na = ""
)

message("2021 primary Medicare FFS Table 1 generation is complete.")
