...
#   - Outputs/5.x_annual_polypharmacy_<years>/
...
# ---- Annual Run Settings ----
# Prefer the session-selected year when this script is sourced by 5.7 or by a
# year-specific R session. Fall back to 2016 only for standalone direct use.
existing_polypharmacy_config <- getOption("frailty.annual_polypharmacy.config", list())
env_analysis_year <- Sys.getenv("FRAILTY_ANALYSIS_YEAR", unset = "")
selected_analysis_years <- if (nzchar(env_analysis_year)) {
  as.integer(env_analysis_year)
} else if (!is.null(existing_polypharmacy_config$analysis_years)) {
  existing_polypharmacy_config$analysis_years
} else {
  2016L
}

selected_analysis_years <- sort(unique(as.integer(selected_analysis_years)))
if (
  length(selected_analysis_years) == 0L ||
    any(is.na(selected_analysis_years)) ||
    any(selected_analysis_years < 2016L | selected_analysis_years > 2025L)
) {
...
options(
  "frailty.annual_polypharmacy.config" = utils::modifyList(
    existing_polypharmacy_config,
    list(analysis_years = selected_analysis_years)
  )
)
