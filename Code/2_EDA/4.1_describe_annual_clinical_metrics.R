...
# score, HIV-related CCW burden summaries, and medical-insurance comparison
# summaries for CFI, Gagne, HIV, and CCW groups. The script keeps patient-level
...
insurance_comparison_expr <- paste0(
  "CASE
     WHEN UPPER(TRIM(mx_insurance_group)) = 'COMMERCIAL' THEN 'Commercial'
     WHEN UPPER(TRIM(mx_insurance_group)) = 'MEDICAID' THEN 'Medicaid'
     WHEN UPPER(TRIM(mx_insurance_group)) = 'MEDICARE'
       AND (
         UPPER(TRIM(mx_insurance_segment)) LIKE '%ADVANTAGE%' OR
         UPPER(TRIM(mx_insurance_segment)) LIKE '%ADV%' OR
         UPPER(TRIM(mx_insurance_segment)) IN ('MA', 'MAPD')
       )
       THEN 'Medicare Advantage'
     WHEN UPPER(TRIM(mx_insurance_group)) = 'MEDICARE'
       AND (
         UPPER(TRIM(mx_insurance_segment)) LIKE '%FFS%' OR
         UPPER(TRIM(mx_insurance_segment)) LIKE '%FEE%' OR
         UPPER(TRIM(mx_insurance_segment)) LIKE '%TRADITIONAL%'
       )
       THEN 'Medicare FFS'
     ELSE NULL
   END"
...
insurance_comparison_strata <- list(
  age_group = age_group_expr,
  sex = sex_expr,
  race_ethnicity = race_expr
)
...
insurance_subgroup_metric_sql <- function(metric_name, value_expr, where_sql) {
  parts <- lapply(
    names(insurance_comparison_strata),
    function(stratum_name) {
      paste0(
        "SELECT
           analysis_year,
           ", sql_string(metric_name), " AS metric,
           ", insurance_comparison_expr, " AS mx_insurance_comparison_group,
           ", sql_string(stratum_name), " AS stratification,
           ", insurance_comparison_strata[[stratum_name]], " AS stratum_value,
           COUNT(*)::BIGINT AS n_person_years,
           AVG(", value_expr, ")::DOUBLE PRECISION AS mean_value,
           STDDEV_SAMP(", value_expr, ")::DOUBLE PRECISION AS sd_value,
           PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY ", value_expr, ") AS p25,
         ...
         WHERE ", analysis_year_filter, "
           AND ", where_sql, "
           AND ", insurance_comparison_expr, " IS NOT NULL
         GROUP BY analysis_year,
           ", insurance_comparison_expr, ",
           ", insurance_comparison_strata[[stratum_name]], "
         HAVING COUNT(*) >= ", min_count
      )
    }
  )
...
hiv_prevalence_by_insurance_subgroup_sql <- function() {
  parts <- lapply(
    names(insurance_comparison_strata),
    function(stratum_name) {
      numerator <- "SUM(CASE WHEN hiv_status = 1 THEN 1 ELSE 0 END)"
      paste0(
        "SELECT
           analysis_year,
           ", insurance_comparison_expr, " AS mx_insurance_comparison_group,
           ...
           CASE WHEN ", numerator, " BETWEEN 1 AND ", min_count - 1L, "
                THEN NULL ELSE ", numerator, "::BIGINT END AS n_hiv_positive,
           CASE WHEN ", numerator, " BETWEEN 1 AND ", min_count - 1L, "
                THEN NULL ELSE ", numerator, "::DOUBLE PRECISION / COUNT(*) END
                AS hiv_positive_prevalence,
           CASE WHEN ", numerator, " BETWEEN 1 AND ", min_count - 1L, "
                THEN TRUE ELSE FALSE END AS small_cell_suppressed
         FROM ", final_identifier, "
         WHERE ", analysis_year_filter, "
           AND hiv_status IS NOT NULL
           AND ", insurance_comparison_expr, " IS NOT NULL
         GROUP BY analysis_year,
           ", insurance_comparison_expr, ",
           ", insurance_comparison_strata[[stratum_name]], "
...
cfi_by_insurance_subgroup <- db_query(
  "Summarizing CFI distribution by medical insurance comparison group and subgroup.",
  insurance_subgroup_metric_sql("CFI", "cfi_score", "cfi_score IS NOT NULL")
)
write_output(
  cfi_by_insurance_subgroup,
  "4.1_cfi_distribution_by_mx_insurance_subgroup.csv"
)
...
gagne_by_insurance_subgroup <- db_query(
  "Summarizing Gagne distribution by medical insurance comparison group and subgroup.",
  insurance_subgroup_metric_sql("Gagne", "gagne_score", "gagne_score IS NOT NULL")
)
write_output(
  gagne_by_insurance_subgroup,
  "4.1_gagne_distribution_by_mx_insurance_subgroup.csv"
)
...
ccw_group_insurance_subgroup_parts <- lapply(
  ccw_group_columns,
  function(column) {
    group_name <- toupper(sub("^index_", "", column))
    numerator <- paste0(
...
ccw_group_insurance_subgroup_prevalence <- db_query(
  "Calculating CCW group prevalence by medical insurance comparison group and subgroup.",
  paste(
    paste0("(", unlist(ccw_group_insurance_subgroup_parts), ")"),
    collapse = "\nUNION ALL\n"
  )
)
write_output(
  ccw_group_insurance_subgroup_prevalence,
  "4.1_ccw_group_prevalence_by_mx_insurance_subgroup.csv"
)

hiv_insurance_subgroup_prevalence <- db_query(
  "Calculating HIV-positive prevalence by medical insurance comparison group and subgroup.",
  hiv_prevalence_by_insurance_subgroup_sql()
)
write_output(
  hiv_insurance_subgroup_prevalence,
...
build_insurance_level_distribution <- function(
  group_expr,
  group_column,
  where_sql,
  message_label
  ...
  db_query(
    ...
    paste0(
      "WITH level_counts AS (
         ",
      paste(unlist(level_parts), collapse = "\nUNION ALL\n"),
      "
       ),
       denominators AS (
         SELECT
           analysis_year,
           mx_insurance_comparison_group,
           stratification,
           stratum_value,
           SUM(n_person_years)::BIGINT AS subgroup_denominator
         FROM level_counts
         GROUP BY
           analysis_year,
           mx_insurance_comparison_group,
       ...
       SELECT
         l.analysis_year,
         l.mx_insurance_comparison_group,
         l.stratification,
         l.stratum_value,
         l.", group_column, ",
         d.subgroup_denominator,
         CASE WHEN l.n_person_years BETWEEN 1 AND ", min_count - 1L, "
              THEN NULL ELSE l.n_person_years END AS n_person_years,
         CASE WHEN l.n_person_years BETWEEN 1 AND ", min_count - 1L, "
              THEN NULL ELSE l.n_person_years::DOUBLE PRECISION /
                d.subgroup_denominator END AS percent_within_subgroup,
         CASE WHEN l.n_person_years BETWEEN 1 AND ", min_count - 1L, "
              THEN TRUE ELSE FALSE END AS small_cell_suppressed
       FROM level_counts l
       INNER JOIN denominators d
         ON l.analysis_year = d.analysis_year
        AND l.mx_insurance_comparison_group = d.mx_insurance_comparison_group
        AND l.stratification = d.stratification
        AND l.stratum_value = d.stratum_value
...
cfi_level_by_insurance_subgroup <- build_insurance_level_distribution(
  group_expr = frailty_expr,
  group_column = "frailty_level",
  where_sql = "cfi_score IS NOT NULL",
  message_label = "Building CFI level distribution by medical insurance comparison group and subgroup."
)
write_output(
  cfi_level_by_insurance_subgroup,
...
gagne_level_by_insurance_subgroup <- build_insurance_level_distribution(
  group_expr = gagne_level_expr,
  group_column = "gagne_level",
  where_sql = "gagne_score IS NOT NULL",
  message_label = "Building Gagne level distribution by medical insurance comparison group and subgroup."
)
write_output(
  gagne_level_by_insurance_subgroup,
