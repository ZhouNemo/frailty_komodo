...
# outputs and, by default, renders the matching HTML reports so a 2025 run does
...
source("Code/2_EDA/4.1_describe_annual_clinical_metrics.R")
source("Code/2_EDA/4.3_prepare_annual_clinical_metrics_visualization_inputs.R")
...
if (isTRUE(render_report)) {
  ...
  descriptive_report_file <- paste0(
  ...
  rmarkdown::render(
    input = "Code/2_EDA/4.2_visualize_annual_clinical_metrics_descriptive_outputs.Rmd",
    output_file = descriptive_report_file,
  ...
  message("Rendered ", file.path(descriptive_output_dir, descriptive_report_file))

  insurance_report_file <- paste0(
    "4.5_clinical_metrics_by_mx_insurance_",
    year_label,
    ".html"
  )

  rmarkdown::render(
    input = "Code/2_EDA/4.5_visualize_clinical_metrics_by_mx_insurance.Rmd",
    output_file = insurance_report_file,
    output_dir = descriptive_output_dir,
    params = list(
      descriptive_output_dir = descriptive_output_dir,
      analysis_year_label = year_label
  ...
  message("Rendered ", file.path(descriptive_output_dir, insurance_report_file))
}
