...
# validation helpers for the normalized 6.x clinical-metrics pipeline. The
...
event_window_sql <- function(config, alias = "ids") {
  if (is.null(config$event_start_date)) {
    return(paste0(
      "event_date BETWEEN ",
      alias,
  ...
  paste0(
    "event_date BETWEEN ",
    sql_string(as.character(config$event_start_date)),
    "::DATE AND ",
    sql_string(as.character(config$event_end_date)),
    "::DATE"
  )
}
