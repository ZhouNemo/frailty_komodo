...
# Project: Frailty_Komoto optimized annual metric IDs
# Author: Nemo Zhou
# Date started: 2026-06-29
# Date last updated: 2026-06-29
#
# ---- Purpose ----
# Prepare or validate the shared patient-year denominator for the optimized 6.x
# clinical-metrics pipeline. The script writes selected years to:
#   - 2_annual_metric_ids
#
# It is restartable by selected year. Patient-level rows remain in Redshift and
# only aggregate integrity checks are printed.
...
id_integrity <- print_query(
  con,
  "Checking optimized metric ID integrity.",
  paste0(
    "SELECT
...
if (
  ...
  stop("The optimized metric ID table failed its integrity check.")
}

print_query(
  con,
  "Checking optimized metric ID year coverage.",
  paste0(
