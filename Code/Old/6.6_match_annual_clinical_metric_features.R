...
# Project: Frailty_Komoto optimized annual clinical metric feature matching
# Author: Nemo Zhou
# Date started: 2026-06-29
# Date last updated: 2026-06-29
#
# ---- Purpose ----
# Match lean patient-year diagnosis/procedure code-presence tables to the
# reviewed CFI, CCW, and Gagne lookup rules. This script does not read raw KRD
# event tables and does not repeat array flattening. It writes selected years to:
...
diagnosis_stage_table <- "clinical_metric_6_diagnosis_lookup_stage"
procedure_stage_table <- "clinical_metric_6_procedure_lookup_stage"
...
if (length(missing_lookup_files) > 0L) {
  stop(
    "Missing optimized clinical metric lookup files:\n",
...
DatabaseConnector::executeSql(
  ...
  paste0(
    "DELETE FROM ", cfi_matches_identifier, "
     ...
     WITH diagnosis_matches AS (
       ...
       INNER JOIN ", diagnosis_stage_identifier, " l
         ON (
           (
             l.match_type = 'exact'
             AND d.diagnosis_code = l.match_value
           )
           OR (
             l.match_type = 'prefix'
             AND LEFT(d.diagnosis_code, LENGTH(l.match_value)) = l.match_value
           )
           OR (
             l.match_type = 'range'
             AND d.diagnosis_code >= l.range_start
             AND (
               (
                 COALESCE(UPPER(l.range_end_inclusive), 'TRUE') = 'FALSE'
                 AND d.diagnosis_code < l.range_end
               )
               OR (
                 COALESCE(UPPER(l.range_end_inclusive), 'TRUE') <> 'FALSE'
                 AND d.diagnosis_code <= l.range_end
     ...
     FROM ", diagnosis_presence_identifier, " d
     INNER JOIN ", diagnosis_stage_identifier, " l
       ON LEFT(d.diagnosis_code, LENGTH(l.match_value)) = l.match_value
     WHERE d.analysis_year IN (", sql_values(config$analysis_years), ")
       AND l.metric = 'CCW'
       AND l.match_type = 'prefix';
...
print_query(
  con,
  "Checking optimized compact feature match counts.",
  paste0(
