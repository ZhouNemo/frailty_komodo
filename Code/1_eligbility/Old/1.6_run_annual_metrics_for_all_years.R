...
run_selected_year <- function(analysis_year) {
  ...
  run_source_stage("Code/1_eligbility/Old/1.4_filter_clinical_metrics_to_non_inpatient_claim_eligible.R")

  options("frailty.patient_geography_join.config" = list(
    analysis_years = analysis_year,
    source_eligibility_table = restricted_eligibility_table
  ))
  run_source_stage("Code/1_eligbility/Old/1.5_join_patient_geography_to_clinical_metrics.R")
...
tryCatch(
  {
    if (isTRUE(run_base_setup)) {
      ...
      run_source_stage("Code/1_eligbility/Old/1.1_build_annual_eligible_population.R")
      run_source_stage("Code/1_eligbility/Old/1.2_check_annual_eligible_population.R")
      run_source_stage("Code/1_eligbility/Old/1.3_join_race_ethnicity_to_eligible_cohort.R")
    }
