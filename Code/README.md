...
The previous CFI and raw-event clinical-metrics processing scripts from `3.*`
through `6.*` are archived in `Code/Old/`. The active clinical-metrics pathway
now follows `Documents/01_CLINICAL_METRICS_DATA_PROCESSING_FLOW.md` and uses:
...
- `6.xx`: active normalized annual clinical-metrics pipeline.
...
## `6.0_normalized_clinical_metrics_helpers.R`

Shared helper file for the active normalized `6.x` clinical-metrics pipeline.
It defines the standard configuration, Redshift connection, SQL quoting,
lookup loading, table validation, batched lookup inserts, and optional
diagnostic event-window overrides. It does not create Redshift tables when
sourced by itself.
...
The default configuration processes 2016 only.

## `6.1_prepare_annual_metric_ids.R`
...
## `6.4_prepare_annual_code_presence.R`

Builds compact patient-year code presence from the normalized event tables. It
joins `2_annual_metric_ids` to `komodo_ext.normalized_dx_events` and
`komodo_ext.normalized_procedure_events` by `patient_id`, restricts
`event_date` to the patient-year window, and writes:
...
Run this after `6.1_prepare_annual_metric_ids.R`.

## `6.5_prepare_annual_hiv_diagnosis_evidence.R`
...
## `6.6_match_annual_clinical_metric_features.R`

Matches the compact code-presence tables to the reviewed CFI, CCW, and Gagne
lookup rules. It writes:
...
Run this after `6.4_prepare_annual_code_presence.R`.

## `6.7_calculate_normalized_annual_cfi_scores.R`
...
Run this after `6.6_match_annual_clinical_metric_features.R`.

## `6.8_calculate_normalized_annual_ccw_variables.R`
...
## `6.9_calculate_normalized_annual_gagne_score.R`
...
## `6.10_calculate_normalized_annual_hiv_status.R`
...
Run this after `6.5_prepare_annual_hiv_diagnosis_evidence.R`.

## `6.11_build_normalized_annual_clinical_metrics.R`
...
Run this after `6.7`, `6.8`, `6.9`, and `6.10` have completed successfully.

## `6.12_check_normalized_annual_clinical_metrics.R`
...
```text
Outputs/6.12_normalized_annual_clinical_metrics_qa.csv
```

Run this after `6.11_build_normalized_annual_clinical_metrics.R`.

## `6.13_run_normalized_annual_clinical_metrics.R`

Run-all wrapper for the normalized `6.x` flow. It sources `6.1`, `6.4`,
`6.5`, `6.6`, `6.7`, `6.8`, `6.9`, `6.10`, `6.11`, and `6.12` in order using
