...
# Code/5.2_export_polypharmacy_unique_ndc11.R and
...
# version. After Outputs/5.3_polypharmacy_ndc11_atc_crosswalk.csv exists, do not
# rerun this script for the same export unless the NDC input file changed, the
...
[CmdletBinding()]
param(
  [string]$InputPath = "Outputs\5.2_polypharmacy_unique_ndc11_2016.txt",
  [string]$CrosswalkOutputPath = "Outputs\5.3_polypharmacy_ndc11_atc_crosswalk.csv",
  [string]$N2cDir = "n2c",
...
if (-not (Test-Path -LiteralPath $inputFullPath)) {
  throw "Missing n2c input file: $inputFullPath. Run Code/5.2_export_polypharmacy_unique_ndc11.R first."
}
...
Write-Host "  Crosswalk for Code/5.3: $crosswalkFullPath"
Write-Host "Next run Code/5.3_stage_polypharmacy_ndc11_atc_crosswalk.R."
