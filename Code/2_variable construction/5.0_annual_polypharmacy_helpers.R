...
get_annual_polypharmacy_config <- function() {
  ...
  config$output_root <- as.character(config$output_root)
  if (length(config$output_root) != 1L || is.na(config$output_root) || !nzchar(config$output_root)) {
    stop("output_root must be one nonempty path.")
  }
  if (is.null(config$output_dir)) {
    config$output_dir <- polypharmacy_default_output_dir(config)
  }
  ...
  if (!is.null(config$unique_ndc_export_path)) {
    ...
    config$unique_ndc_export_path <- polypharmacy_default_unique_ndc_export_path(config)
  }
  if (!is.null(config$crosswalk_input_path)) {
    config$crosswalk_input_path <- as.character(config$crosswalk_input_path)
  } else {
    config$crosswalk_input_path <- polypharmacy_default_crosswalk_input_path(config)
  }
...
polypharmacy_default_output_dir <- function(config) {
  file.path(
    config$output_root,
    paste0("5.x_annual_polypharmacy_", polypharmacy_year_label(config))
  )
}
...
polypharmacy_default_crosswalk_input_path <- function(config) {
  file.path(
    config$output_dir,
    paste0(
      "5.3_polypharmacy_ndc11_atc_crosswalk_",
      polypharmacy_year_label(config),
      ".csv"
    )
  )
}
