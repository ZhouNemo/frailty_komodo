...
# final-table completeness, duplicate keys, report-ready Table 1 summaries, and
# subgroup-within-prescription-insurance polypharmacy summaries. Patient-level
# rows are not printed. Small cells are suppressed before CSV export.
...
fills_duplicate_rows <- qa_results$duplicates$duplicate_rows[
  qa_results$duplicates$qa_check == "fills_table"
]
qa_results$fill_duplicate_key_summary <- if (
  length(fills_duplicate_rows) > 0L && any(fills_duplicate_rows != 0)
) {
  DBI::dbGetQuery(
    con,
    paste0(
      "WITH duplicate_keys AS (
         SELECT
           patid,
           analysis_year,
           COALESCE(pharmacy_event_id, fill_date::VARCHAR || '|' || ndc11) AS fill_key,
           COUNT(*)::BIGINT AS n_rows,
           COUNT(DISTINCT fill_date)::BIGINT AS n_fill_dates,
           COUNT(DISTINCT ndc11)::BIGINT AS n_ndc11,
           COUNT(DISTINCT days_supply)::BIGINT AS n_days_supply
         FROM ", fills_identifier, "
         WHERE analysis_year IN (", sql_values(config$analysis_years), ")
         GROUP BY
           patid,
       ...
       SELECT
         'fill_duplicate_key_summary' AS qa_check,
         analysis_year,
         COUNT(*)::BIGINT AS n_duplicate_keys,
         SUM(n_rows)::BIGINT AS n_rows_in_duplicate_keys,
         SUM(n_rows - 1)::BIGINT AS n_excess_rows,
         SUM(CASE WHEN n_fill_dates > 1 OR n_ndc11 > 1 OR n_days_supply > 1
           THEN 1 ELSE 0 END)::BIGINT AS n_analytic_discordant_keys
       FROM duplicate_keys
       GROUP BY analysis_year
       ORDER BY analysis_year"
    )
  ...
  data.frame(
    ...
    n_rows_in_duplicate_keys = numeric(),
    n_excess_rows = numeric(),
    n_analytic_discordant_keys = numeric()
  )
}
...
polypharmacy_insurance_subgroup_path <- file.path(
  config$output_dir,
...
utils::write.csv(
  polypharmacy_insurance_subgroup,
  polypharmacy_insurance_subgroup_path,
  row.names = FALSE
)
